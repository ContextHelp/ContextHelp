...
func TestMigration014_ColumnsExist(t *testing.T) {
	d := newTestDriver(t)
	...
	for _, col := range []string{"remind_at", "reminded_at"} {
		...
		if err != nil {
			t.Fatalf("column %q not found after migration 014: %v", col, err)
		}
	}
}

func TestMigration014_SetAndClearReminder(t *testing.T) {
	d := newTestDriver(t)
	ctx := context.Background()
