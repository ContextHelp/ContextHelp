...
As a knowledge worker, I want to import data from OneDrive and SharePoint document libraries so I can make existing knowledge searchable in ctxt without manual copy/paste.
...
- [ ] Importer accepts OneDrive and SharePoint document libraries as input and validates required configuration.
- [ ] Auth flow supports OAuth 2.0 with Microsoft Entra ID.
- [ ] Importer supports first-time backfill and incremental sync using Graph delta queries.
- [ ] Imported records map to object types: Office files, PDFs, text documents.
...
- Example: ctxt import onedrive --profile default --since 2026-01-01
...
- Default pipeline for this story: import.onedrive
...
- POST /api/v1/importers/onedrive/run
...
- [ ] CLI: `ctxt import onedrive --profile default --since 2026-01-01` sends `"importer_key": "onedrive"`, `"profile": "default"`, and `"since": "2026-01-01"` in the request payload; server creates a run record with all three fields — GET /api/v1/importers/runs/{run_id} confirms.
...
- [ ] Import run succeeds with representative OneDrive fixture; produced KnowledgeObjects contain item URL, title, and file type fields.
- [ ] Stored KnowledgeObjects have provenance metadata: `source=onedrive`, `external_id` matching the Graph item ID, and `import_timestamp` set to ingest time.
...
- [ ] Incremental sync: uses Graph delta query token; second run fetches only items changed after the stored checkpoint — `skipped` count equals unchanged items.
- [ ] Re-running same source data does not create duplicates; dedup table row count unchanged.
- [ ] Auth errors (invalid or expired Entra ID token) are returned with actionable guidance and `error_type=auth`.
...
- [Importer Plan](../../plans/2026-02-18-importer-onedrive-plan.md) — Source-specific plan
