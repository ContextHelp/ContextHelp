...
As a knowledge worker, I want to import data from Safari bookmarks exports so I can make existing knowledge searchable in ctxt without manual copy/paste.
...
- [ ] Importer accepts Safari bookmarks exports as input and validates required configuration.
- [ ] Auth flow supports local export file input (no OAuth).
- [ ] Importer supports first-time backfill and incremental sync using file hash and bookmark key comparison.
- [ ] Imported records map to object types: bookmark URL, title, folder hierarchy.
...
- Example: ctxt import safari --file ./bookmarks.html --profile default
...
- Default pipeline for this story: import.safari
...
- POST /api/v1/importers/safari/run
...
- [ ] CLI: `ctxt import safari --file ./bookmarks.html --profile default` sends `"importer_key": "safari"`, `"file"` path, and `"profile": "default"` in the request payload; server creates a run record with all three fields — GET /api/v1/importers/runs/{run_id} confirms.
...
- [ ] Import run succeeds with representative Safari bookmarks export fixture; produced KnowledgeObjects contain `url`, `title`, and `folder_hierarchy` fields.
- [ ] Stored KnowledgeObjects have provenance metadata: `source=safari`, `external_id` matching the bookmark's original key, and `import_timestamp` set to ingest time.
...
- [ ] Incremental sync: second run comparing file hash and bookmark keys imports zero new records when data is unchanged; `skipped` count equals previous `imported` count.
...
- [Importer Plan](../../plans/2026-02-18-importer-safari-plan.md) — Source-specific plan
