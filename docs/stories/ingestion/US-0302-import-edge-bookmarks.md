...
As a knowledge worker, I want to import data from Edge bookmarks HTML exports so I can make existing knowledge searchable in ctxt without manual copy/paste.
...
- [ ] Importer accepts Edge bookmarks HTML exports as input and validates required configuration.
- [ ] Auth flow supports local export file input (no OAuth).
- [ ] Importer supports first-time backfill and incremental sync using file hash plus bookmark timestamps when available.
- [ ] Imported records map to object types: bookmark URL, title, folder path, add date.
...
- Example: ctxt import edge --file ./bookmarks.html --profile default
...
- Default pipeline for this story: import.edge
...
- POST /api/v1/importers/edge/run
...
- [ ] CLI: `ctxt import edge --file ./bookmarks.html --profile default` sends `"importer_key": "edge"`, `"file"` path, and `"profile": "default"` in the request payload; server creates a run record with all three fields — GET /api/v1/importers/runs/{run_id} confirms.
...
- [ ] Import run succeeds with representative Edge bookmarks HTML fixture; produced KnowledgeObjects contain `url`, `title`, `folder_path`, and `add_date` fields.
- [ ] Stored KnowledgeObjects have provenance metadata: `source=edge`, `external_id` matching the bookmark's original ID, and `import_timestamp` set to ingest time.
...
- [Importer Plan](../../plans/2026-02-18-importer-edge-plan.md) — Source-specific plan
