# US-0308: Import Dropbox Content
...
As a knowledge worker, I want to import data from Dropbox files and folders so I can make existing knowledge searchable in ctxt without manual copy/paste.
...
- [ ] Importer accepts Dropbox files and folders as input and validates required configuration.
- [ ] Auth flow supports OAuth 2.0 with Dropbox.
- [ ] Importer supports first-time backfill and incremental sync using list_folder cursor sync.
- [ ] Imported records map to object types: text/markdown documents, PDFs, metadata.
...
- Example: ctxt import dropbox --profile default --since 2026-01-01
...
- Default pipeline for this story: import.dropbox
...
- POST /api/v1/importers/dropbox/run
...
- [ ] CLI: `ctxt import dropbox --profile default --since 2026-01-01` sends `"importer_key": "dropbox"`, `"profile": "default"`, and `"since": "2026-01-01"` in the request payload; server creates a run record with all three fields — GET /api/v1/importers/runs/{run_id} confirms.
...
- [ ] Import run succeeds with representative Dropbox fixture; produced KnowledgeObjects contain file name, path, and content type fields.
- [ ] Stored KnowledgeObjects have provenance metadata: `source=dropbox`, `external_id` matching the Dropbox item ID, and `import_timestamp` set to ingest time.
...
- [ ] Incremental sync: uses list_folder cursor; second run fetches only items changed after the stored cursor — `skipped` count equals unchanged items.
...
- [Importer Plan](../../plans/2026-02-18-importer-dropbox-plan.md) — Source-specific plan
