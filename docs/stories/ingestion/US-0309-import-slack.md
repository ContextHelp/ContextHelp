...
# US-0309: Import Slack Workspace
...
As a knowledge worker, I want to import data from Slack workspace export bundles or API data so I can make existing knowledge searchable in ctxt without manual copy/paste.
...
- [ ] Importer accepts Slack workspace export bundles or API data as input and validates required configuration.
- [ ] Auth flow supports workspace export upload for MVP, OAuth for API mode.
- [ ] Importer supports first-time backfill and incremental sync using export window tracking or conversations cursor.
- [ ] Imported records map to object types: messages, threads, channels, attachment metadata.
...
- Example: ctxt import slack --file ./slack-export.zip --profile default
...
- Default pipeline for this story: import.slack
...
- POST /api/v1/importers/slack/run
...
- [ ] CLI: `ctxt import slack --file ./slack-export.zip --profile default` sends `"importer_key": "slack"`, `"file"` path, and `"profile": "default"` in the request payload; server creates a run record with all three fields — GET /api/v1/importers/runs/{run_id} confirms.
...
- [ ] Import run succeeds with representative Slack export fixture; produced KnowledgeObjects contain message text, channel name, and timestamp fields.
- [ ] Stored KnowledgeObjects have provenance metadata: `source=slack`, `external_id` matching the Slack message timestamp/ID, and `import_timestamp` set to ingest time.
...
- [ ] Incremental sync: tracks export window or conversations cursor; second run skips already-ingested messages — `skipped` count equals previously imported message count.
- [ ] Re-running same source data does not create duplicates; dedup table row count unchanged.
- [ ] Auth errors (invalid export file or expired OAuth token) are returned with actionable guidance and `error_type=auth`.
...
- [Importer Plan](../../plans/2026-02-18-importer-slack-plan.md) — Source-specific plan
