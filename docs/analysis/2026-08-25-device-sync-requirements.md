...
**T1 — Hub: remote authoritative primary + local fallback (the priority deployment).** An owner-hosted, internet-reachable dpkms instance is the designated primary — the always-on replica every other device syncs against. This is not a hypothetical: `docs/architecture/ambient-capture.md` already treats "dpkms is often remote" as the substrate's central constraint, with `ctxd` on the local machine buffering enqueues toward a remote dpkms. The hub composes with the inbound-auth surface (ADR-023 Layer 2–4: token auth, TLS, device authentication) because it is by definition network-exposed. A local instance mirrors the hub while connected, takes writes while disconnected (capture *and* curation — status changes, tags, edits), and reconciles on reconnect.
...
- **No data loss by default:** when a deterministic merge rule must pick a winner between two concurrent legitimate writes, the loser is *preserved and surfaced* (conflict copy / review queue — semantics in the conflict note), never silently discarded. Deterministic-and-lossy is acceptable only for values that are cheap to regenerate or semantically idempotent.
...
| S6 | **Internet attacker vs. the hub** (T1) | exposed API | The hub is a public-bind dpkms instance: daemon-only policy boundary mandatory (federation addendum risk 1), ADR-023 auth on every endpoint, sync endpoints additionally require device-key auth — a leaked API token must not grant replica-set membership. |
