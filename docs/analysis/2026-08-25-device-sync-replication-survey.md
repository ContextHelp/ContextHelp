...
Append a `sync_log` table in the instance DB, written **in the same transaction** as every mutation at the storage/service choke point (§0.6): `(seq INTEGER autoincrement, device_id, hlc, op_type, entity_kind, entity_id, payload JSON)` — plus causal metadata per the fleet's causality encoding (conflict note §0.1: per-op origin sequence-vector, or a replicated apply-record stream; HLC alone is a total order, not a happens-before test, so the merge layer's ordered-vs-concurrent branch needs one of the two). Ops are *semantic*, not physical: `object.create`, `object.edit` (content supersession), `object.set_status`, `object.add_node`, `edge.add`, `edge.remove`, `entity.enrich`, `*.delete` (tombstone), `reinforce`. Exchange: peers track, per remote device, the highest contiguous `seq` acked (a device-sequence vector — O(roster) integers total, not per-row); a sync ships ops the peer lacks, in order per origin device. Receiver applies each op through the per-type merge rules (candidate B's semantics), which are commutative/idempotent by construction, then records it — application and ack commit atomically (§0.1).
...
- **Storage overhead:** O(ops until roster-acked GC) for the log — **plus a fixed per-row floor an earlier draft of this scoring omitted**: LWW must still be decidable after the deciding ops are pruned, so every LWW field group persists an HLC column (`objects.content_hlc/status_hlc/…`, `edges.attr_hlc` — conflict note §1.2), and OR-set elements persist add-dots per the fleet's causality option. That is a bounded subset of B's metadata (per-group clocks, not per-field clock state on every path), so the C-vs-B storage contrast narrows to "log + group clocks + dots" vs "full per-field-group state metadata + dots"; C's decisive margins are wire shape, GC, and audit — not zero row metadata. The log is prunable: an op is GC-able once every roster device has acked past it (same causal-stability rule as tombstones — [2026-08-04-retraction-propagation.md](2026-08-04-retraction-propagation.md) §3 — computable exactly over a known roster; tombstone ops respect the retention floor; under the apply-record causality option, the record stream GCs by the same rule).
- **Auditability:** the log *is* an attributable history — every op attributable to a device key through its origin-signed batch (the identity/transport note fixes the attribution unit), dovetailing with the append-only `audit_log` (`migrations/011_audit_log.sql`) and AC12. No other candidate gives this for free.
...
- **Anti-entropy backstop:** periodic Merkle/range digest comparison over replicated truth — per table, leaves keyed by row id, each leaf hashing the row's replicated columns **plus its persisted merge clocks** (the per-field-group HLC columns and set-element dots of conflict note §1.2; there is no single per-row HLC to hash) — detects silent divergence (bug, restored backup, pre-sync history) and drives the epoch full-resync path from requirements S4. The fleet's conflict-copy option (conflict note §0.2) fixes whether conflict artifacts (`sync_conflicts`, conflict copies) sit inside the digest (replicated-truth option) or outside it (local-only option). State-based reconciliation as the *repair* tool, op-based as the *normal* path. This is the CouchDB/Syncthing lesson: every production op-shipping system grows a state-comparison repair mode; design it in.
...
| Storage overhead | ++ (2 cols) | − (per-field meta) | **+ (O(ops) log GC'd by roster ack, + per-row group-HLC/dot floor)** | + | − (per-column meta) |
