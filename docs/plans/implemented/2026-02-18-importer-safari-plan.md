# Safari Bookmarks Importer Plan
...
## Goal
Ship an out-of-box Safari Bookmarks importer that supports first-time backfill and repeat sync into dPKMS/ctxt with stable deduplication and operational visibility.
...
## Data Surface
- **Primary surface:** Bookmarks HTML export from Safari
- **Auth model:** Local file input (no OAuth for MVP)
- **Incremental sync:** File hash + per-bookmark key

## MVP Scope
- Import object types: bookmark URL, title, folder hierarchy
...
- Out of scope (MVP): Keychain/password import and reading Safari internals
...
## API/CLI Touchpoints
- CLI: ctxt import safari --profile <name> [--since ...] [--max-items N] [--dry-run]
- API: POST /api/v1/importers/safari/run and GET /api/v1/importers/runs/{id}
- Storage: importer checkpoint table keyed by (profile, source, account)

## Key Risks
- Safari export inconsistencies across macOS versions
- iCloud merge duplicates if users import multiple exports
