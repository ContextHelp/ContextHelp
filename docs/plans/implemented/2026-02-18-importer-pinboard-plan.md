...
## Goal
Ship an out-of-box Pinboard importer that supports first-time backfill and repeat sync into dPKMS/ctxt with stable deduplication and operational visibility.
...
## Data Surface
- **Primary surface:** Pinboard API export (JSON/XML) or bookmark export file
- **Auth model:** API token or local export file
- **Incremental sync:** Bookmark updated timestamp cursor

## MVP Scope
- Import object types: bookmarks, tags, description, extended notes
...
- Out of scope (MVP): Real-time webhook-driven ingestion
...
## API/CLI Touchpoints
- CLI: ctxt import pinboard --profile <name> [--since ...] [--max-items N] [--dry-run]
- API: POST /api/v1/importers/pinboard/run and GET /api/v1/importers/runs/{id}
- Storage: importer checkpoint table keyed by (profile, source, account)

## Key Risks
- API limits and occasional service instability
- Tag normalization differences versus local bookmark formats
