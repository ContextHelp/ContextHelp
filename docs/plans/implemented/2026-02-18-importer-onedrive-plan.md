...
## Goal
Ship an out-of-box OneDrive importer that supports first-time backfill and repeat sync into dPKMS/ctxt with stable deduplication and operational visibility.
...
## Data Surface
- **Primary surface:** Microsoft Graph Drive API (OneDrive/SharePoint document libraries)
- **Auth model:** OAuth 2.0 (Microsoft Entra ID)
- **Incremental sync:** Graph delta queries for drives/items

## MVP Scope
- Import object types: Office docs, PDFs, text files, folder metadata
...
- Out of scope (MVP): Full SharePoint site architecture modeling
...
## API/CLI Touchpoints
- CLI: ctxt import onedrive --profile <name> [--since ...] [--max-items N] [--dry-run]
- API: POST /api/v1/importers/onedrive/run and GET /api/v1/importers/runs/{id}
- Storage: importer checkpoint table keyed by (profile, source, account)

## Key Risks
- Delta tokens can expire and require safe replay
- Tenant-specific permission boundaries complicate support
