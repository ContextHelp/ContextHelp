...
## Goal
Ship an out-of-box Google Drive importer that supports first-time backfill and repeat sync into dPKMS/ctxt with stable deduplication and operational visibility.
...
## Data Surface
- **Primary surface:** Google Drive API + Docs export endpoints
- **Auth model:** OAuth 2.0 (Google)
- **Incremental sync:** Drive changes API token + modifiedTime fallback

## MVP Scope
- Import object types: Google Docs, Sheets, Slides (exported), PDFs, text files
...
- Out of scope (MVP): Domain-wide delegation admin workflows
...
## API/CLI Touchpoints
- CLI: ctxt import gdrive --profile <name> [--since ...] [--max-items N] [--dry-run]
- API: POST /api/v1/importers/gdrive/run and GET /api/v1/importers/runs/{id}
- Storage: importer checkpoint table keyed by (profile, source, account)

## Key Risks
- Export fidelity differences between Google native docs and downloaded formats
- API quotas and per-user throttling during large backfills
