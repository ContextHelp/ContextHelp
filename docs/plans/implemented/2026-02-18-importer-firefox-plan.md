# Firefox Bookmarks Importer Plan
...
## Goal
Ship an out-of-box Firefox Bookmarks importer that supports first-time backfill and repeat sync into dPKMS/ctxt with stable deduplication and operational visibility.
...
- **Incremental sync:** File hash + bookmark add_date/last_modified

## MVP Scope
- Import object types: bookmark URL, title, folder path, tags when present
...
- Out of scope (MVP): Direct places.sqlite ingestion
...
## API/CLI Touchpoints
- CLI: ctxt import firefox --profile <name> [--since ...] [--max-items N] [--dry-run]
- API: POST /api/v1/importers/firefox/run and GET /api/v1/importers/runs/{id}
- Storage: importer checkpoint table keyed by (profile, source, account)

## Key Risks
- Firefox export may omit fields users expect from internal DB
- Large bookmark trees can stress naive recursive parsers
