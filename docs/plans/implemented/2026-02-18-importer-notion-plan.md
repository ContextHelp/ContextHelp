...
## Goal
Ship an out-of-box Notion importer that supports first-time backfill and repeat sync into dPKMS/ctxt with stable deduplication and operational visibility.
...
## Data Surface
- **Primary surface:** Notion public API (pages/blocks/databases)
- **Auth model:** OAuth 2.0 (Notion integration token)
- **Incremental sync:** Last edited timestamps + cursor pagination

## MVP Scope
- Import object types: pages, databases, block content, page properties
...
- Out of scope (MVP): Two-way sync and rich block rendering parity
...
## API/CLI Touchpoints
- CLI: ctxt import notion --profile <name> [--since ...] [--max-items N] [--dry-run]
- API: POST /api/v1/importers/notion/run and GET /api/v1/importers/runs/{id}
- Storage: importer checkpoint table keyed by (profile, source, account)

## Key Risks
- Deep block trees can exceed naive recursion limits
- Permission-scoped partial content can look like missing data
