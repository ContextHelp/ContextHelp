...
## Goal
Ship an out-of-box Evernote importer that supports first-time backfill and repeat sync into dPKMS/ctxt with stable deduplication and operational visibility.
...
## Data Surface
- **Primary surface:** ENEX export files (plus optional HTML export)
- **Auth model:** Local file input (MVP)
- **Incremental sync:** ENEX note GUID + updated timestamp

## MVP Scope
- Import object types: notes, notebooks, tags, resources/attachments metadata
...
- Out of scope (MVP): Direct cloud API sync beyond export-based import
...
## API/CLI Touchpoints
- CLI: ctxt import evernote --profile <name> [--since ...] [--max-items N] [--dry-run]
- API: POST /api/v1/importers/evernote/run and GET /api/v1/importers/runs/{id}
- Storage: importer checkpoint table keyed by (profile, source, account)

## Key Risks
- ENEX resource blobs can be large and memory-intensive
- Embedded HTML cleanup needed for reliable text extraction
