...
## Goal
Ship an out-of-box Slack importer that supports first-time backfill and repeat sync into dPKMS/ctxt with stable deduplication and operational visibility.
...
## Data Surface
- **Primary surface:** Slack workspace exports and/or Slack Web API
- **Auth model:** Workspace export upload (MVP) + OAuth for API mode
- **Incremental sync:** Export timestamp window or conversations.history cursor

## MVP Scope
- Import object types: messages, threads, channel metadata, basic attachments metadata
...
- Out of scope (MVP): Private-channel export in orgs without required admin access
...
## API/CLI Touchpoints
- CLI: ctxt import slack --profile <name> [--since ...] [--max-items N] [--dry-run]
- API: POST /api/v1/importers/slack/run and GET /api/v1/importers/runs/{id}
- Storage: importer checkpoint table keyed by (profile, source, account)

## Key Risks
- Export availability differs by plan and admin permissions
- Message redactions/deletions create reconciliation edge cases
