# Logseq (requested as 'loqseq') Importer Plan
...
## Goal
Ship an out-of-box Logseq (requested as 'loqseq') importer that supports first-time backfill and repeat sync into dPKMS/ctxt with stable deduplication and operational visibility.
...
## Data Surface
- **Primary surface:** Local markdown/journal graph directory
- **Auth model:** Local filesystem access
- **Incremental sync:** File mtime + block UUID tracking

## MVP Scope
- Import object types: journals, pages, block hierarchy, page references
...
- Out of scope (MVP): Plugin-specific block properties beyond core Logseq format
...
## API/CLI Touchpoints
- CLI: ctxt import logseq --profile <name> [--since ...] [--max-items N] [--dry-run]
- API: POST /api/v1/importers/logseq/run and GET /api/v1/importers/runs/{id}
- Storage: importer checkpoint table keyed by (profile, source, account)

## Key Risks
- Block-level identity must remain stable across file rewrites
- Journal/page merge behavior can create duplicate references
