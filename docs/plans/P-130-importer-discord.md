...
## Goal
Ship an out-of-box Discord importer that supports first-time backfill and repeat sync into dPKMS/ctxt with stable deduplication and operational visibility.
...
## Data Surface
- **Primary surface:** Discord message export file (MVP) or Bot API (approved servers)
- **Auth model:** Export upload (MVP); bot token for API mode
- **Incremental sync:** Message snowflake ID cursor

## MVP Scope
- Import object types: messages, threads, channel metadata, author references
...
- Out of scope (MVP): User-account scraping or ToS-unsafe collection paths
...
## API/CLI Touchpoints
- CLI: ctxt import discord --profile <name> [--since ...] [--max-items N] [--dry-run]
- API: POST /api/v1/importers/discord/run and GET /api/v1/importers/runs/{id}
- Storage: importer checkpoint table keyed by (profile, source, account)

## Key Risks
- API terms and bot permissions constrain coverage
- Large servers require strict rate-limit handling
