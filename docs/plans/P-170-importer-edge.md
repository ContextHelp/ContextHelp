...
## Goal
Ship an out-of-box Edge Bookmarks importer that supports first-time backfill and repeat sync into dPKMS/ctxt with stable deduplication and operational visibility.
...
## API/CLI Touchpoints
- CLI: ctxt import edge --profile <name> [--since ...] [--max-items N] [--dry-run]
- API: POST /api/v1/importers/edge/run and GET /api/v1/importers/runs/{id}
- Storage: importer checkpoint table keyed by (profile, source, account)

## Key Risks
- Multiple export variants depending on Edge channel/version
- Duplicate folder names causing path collisions
