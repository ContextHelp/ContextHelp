...
## Goal
Ship an out-of-box Obsidian Vault importer that supports first-time backfill and repeat sync into dPKMS/ctxt with stable deduplication and operational visibility.
...
## Data Surface
- **Primary surface:** Local markdown vault directory
- **Auth model:** Local filesystem access
- **Incremental sync:** File mtime + content hash checkpoints

## MVP Scope
- Import object types: notes, frontmatter, wikilinks, tags, attachments refs
...
- Out of scope (MVP): Realtime bi-directional sync with conflict resolution
...
## API/CLI Touchpoints
- CLI: ctxt import obsidian --profile <name> [--since ...] [--max-items N] [--dry-run]
- API: POST /api/v1/importers/obsidian/run and GET /api/v1/importers/runs/{id}
- Storage: importer checkpoint table keyed by (profile, source, account)

## Key Risks
- Symlinked vault paths can produce duplicate traversal
- Nonstandard plugin frontmatter fields may break strict parsers
