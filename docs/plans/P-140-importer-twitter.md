...
## Goal
Ship an out-of-box Twitter/X importer that supports first-time backfill and repeat sync into dPKMS/ctxt with stable deduplication and operational visibility.
...
## Data Surface
- **Primary surface:** User archive export (MVP) and API v2 where available
- **Auth model:** Archive upload (MVP); OAuth/API key for API mode
- **Incremental sync:** Tweet ID/timestamp cursor

## MVP Scope
- Import object types: posts, threads, media links, profile metadata
...
- Out of scope (MVP): Full enterprise firehose ingestion
...
## API/CLI Touchpoints
- CLI: ctxt import twitter --profile <name> [--since ...] [--max-items N] [--dry-run]
- API: POST /api/v1/importers/twitter/run and GET /api/v1/importers/runs/{id}
- Storage: importer checkpoint table keyed by (profile, source, account)

## Key Risks
- API access tiers are volatile and can change constraints quickly
- Archive schema versions vary across export generations
