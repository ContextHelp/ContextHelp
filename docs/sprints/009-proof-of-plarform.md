...
## 3b. dPKMS Plugin Extensions Team (The Ecosystem)

Focus: Two missing plugin extension point categories — registry providers and output generators.
Why: Sprint 005 defined `PluginPipelineStep`, `PluginCommand`, and `PluginSemanticAugmentor`. The Skeleton 9 roadmap requires the full plugin surface area to be proven in a reference integration sprint.

### Task 3b.1: Registry Provider Plugin Interface

- Define `PluginRegistryProvider` interface:
  ```go
  type PluginRegistryProvider interface {
      ...
      // ResolveEntity looks up a single entity by slug.
      ResolveEntity(ctx context.Context, namespace, slug string) (*Entity, error)
  }
  ```
- Wire into the Plugin Loader (Sprint 005 Task 1.2): plugins declaring `type: registry_provider` in `manifest.yaml` are loaded as registry providers alongside HTTP registries.
- The existing registry sync logic must iterate all providers (HTTP + plugin-provided) uniformly.
- Reference plugin: implement a `local-registry-plugin` Go binary that reads from a directory of YAML files — proves the interface without requiring a remote server.

### Task 3b.2: Output Generator Plugin Interface

- Define `PluginOutputGenerator` interface:
  ```go
  type PluginOutputGenerator interface {
      // Name returns the output format name (e.g., "obsidian-md", "notion-export").
      Name() string
      // Accepts returns true if this generator can handle the given object.
      Accepts(obj KnowledgeObject) bool
      // Generate renders the object to the desired format.
      Generate(ctx context.Context, obj KnowledgeObject, opts OutputOptions) ([]byte, error)
  }
  ```
- Wire into `ctxt export` and `ctxt open`: if a plugin output generator is loaded and `--format <name>` matches, delegate to the plugin.
- Reference plugin: implement a `markdown-export-plugin` that writes Obsidian-compatible Markdown with frontmatter (title, tags, entities, backlinks).
- Config:
  ```yaml
  plugins:
    markdown-export:
...
## 3c. dPKMS Security CI Team (The Auditor)

Focus: Dev-time and CI/CD security tooling — catching vulnerabilities and leaked secrets before they reach production.
Why: Sprint 008 handles runtime trust. This sprint handles the supply chain: the code and its dependencies must be continuously audited as part of every build.

### Task 3c.1: Pre-Commit Secret Scanning (gitleaks)

- Add `.gitleaks.toml` configuration to the repository root:
  - Allow-list test fixture patterns.
  - Custom rules for `CH_REGISTRY_TOKEN_*` and `CTXT_*` env-var patterns.
- Add `gitleaks` to the Makefile: `make security-scan` runs a full repo scan.
- Document setup in SECURITY.md: `brew install gitleaks && make install-hooks`.
- Pre-commit hook script at `.git/hooks/pre-commit` (generated by `make install-hooks`):
  ```bash
  #!/usr/bin/env bash
  gitleaks protect --staged --no-banner || exit 1
  ```
- CI: add `gitleaks detect` step to GitHub Actions workflow (runs on every PR).

### Task 3c.2: Dependency Vulnerability Scanning (nancy + govulncheck)

- Add `govulncheck` (official Go vulnerability scanner) to CI:
  ```yaml
  - name: govulncheck
    run: govulncheck ./...
  ```
- Add `nancy` for Sonatype OSS Index scanning of `go.sum`:
  ```yaml
  - name: nancy
    run: go list -json -deps ./... | nancy sleuth
  ```
- Both run on every PR and nightly cron.
- Fail build on `CRITICAL` or `HIGH` severity findings.
- Document suppression process for false positives (`.nancy-ignore` file).

### Task 3c.3: Static Security Analysis (gosec)
...
- Add `gosec` to CI:
  ...
  ```
- Add `.gosec.yaml` with project-specific exclusions (e.g., G304 for intentional dynamic file reads).
- Integrate into `make lint` alongside `golangci-lint`.
- Add `gosec` to the `golangci-lint` config as a linter (via `golangci-lint run --enable gosec`).

### Task 3c.4: Container Security Scanning (trivy)

- Add `trivy` scan to CI for the Docker image:
  ```yaml
  - name: trivy image scan
    run: trivy image --exit-code 1 --severity CRITICAL contexthelp/dpkms:latest
  ```
- Scan both the final image and the Dockerfile for misconfigurations.
- Nightly scan of published images to catch newly disclosed CVEs.
- Block release pipeline on `CRITICAL` findings.
