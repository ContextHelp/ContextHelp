...
Examples:
  ...
  ctxt detector add --kind extension --pattern .pdf --pipeline doc.pdf --name "PDF files"

  # Add a URL-pattern detector
  ctxt detector add --kind url_pattern --pattern "github\\.com" --pipeline doc.code --name "GitHub URLs"

  # List all detectors
  ctxt detector list

  # Disable a detector
  ctxt detector disable <id>

  # Enable a detector
  ctxt detector enable <id>

  # Remove a detector
  ctxt detector remove <id>`,
}
...
func init() {
	...
	detectorAddCmd.Flags().String("kind", "", "detector kind: extension, url_pattern, content_test (required)")
	detectorAddCmd.Flags().String("pattern", "", "pattern to match (extension like .pdf, or regexp)")
	detectorAddCmd.Flags().String("pipeline", "", "pipeline name to route to (required)")
	detectorAddCmd.Flags().String("name", "", "human-readable label")
	detectorAddCmd.Flags().Int("priority", 100, "priority (lower = higher, default 100)")
