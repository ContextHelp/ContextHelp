...
// registryYAML is the shape we accept for a registry YAML file.
// We use a loose struct so we can report all issues at once.
...
Examples:
  ctxt dev validate-registry path/to/registry.yaml
  ctxt dev validate-registry registry.yaml --output json`,
...
func runDevValidateRegistry(cmd *cobra.Command, args []string) error {
	...
	// Taxonomy namespace slugs + duplicates.
	seenNS := map[string]int{}
	for i, t := range reg.Taxonomy {
		ns := t.Namespace
		if !slugRE.MatchString(ns) {
			issues = append(issues, fmt.Sprintf("taxonomy[%d]: invalid slug %q (must match [a-z0-9-])", i, ns))
		}
		seenNS[ns]++
	}
	for ns, count := range seenNS {
		if count > 1 {
			issues = append(issues, fmt.Sprintf("taxonomy: duplicate namespace %q (%d occurrences)", ns, count))
		}
	}

	// Step name slugs + duplicates.
	seenStep := map[string]int{}
	for i, s := range reg.Steps {
		if !slugRE.MatchString(s.Name) {
			issues = append(issues, fmt.Sprintf("steps[%d]: invalid slug %q (must match [a-z0-9-])", i, s.Name))
		}
		seenStep[s.Name]++
	}
	for name, count := range seenStep {
		if count > 1 {
			issues = append(issues, fmt.Sprintf("steps: duplicate name %q (%d occurrences)", name, count))
		}
	}
...
Examples:
  ctxt dev init-plugin my-plugin
  ctxt dev init-plugin sentiment --dir ./custom-plugins`,
	Args: cobra.ExactArgs(1),
	RunE: runDevInitPlugin,
}

func init() {
	devInitPluginCmd.Flags().String("dir", "plugins", "parent directory for the new plugin")
}
...
var devGenDocsCmd = &cobra.Command{
	...
	Long: `Generate Markdown reference documentation for all ctxt commands.
...
Examples:
  ctxt dev gen-docs
  ctxt dev gen-docs --dir /tmp/ctxt-docs`,
	RunE: runDevGenDocs,
}

func init() {
	devGenDocsCmd.Flags().String("dir", "docs/cli", "output directory for generated docs")
}
...
func runDevGenDocs(cmd *cobra.Command, args []string) error {
	...
	// Generate markdown docs for the full cobra command tree.
