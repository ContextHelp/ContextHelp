...
//
// executeCommand's resetAllFlags handles the cobra side (resetFlag calls
// SliceValue.Replace(nil) for slice flags — a plain Set(DefValue) would
// APPEND on a stringArray). But kit's Root keeps its own viper, which
// ConfigArgs consults, so it must be cleared separately or a stale -c
// leaks into the next in-process run.
...
// into a tempdir. Without this an in-process CLI test would read the
// developer's real config — the exact hazard this file guards against.
...
// runWithConfigFlag drives the real cobra tree so the assertion covers the
// whole path: OnInitialize(initConfig) → PersistentPreRunE → Execute.
...
// data-safety defect: `dpkms housekeeping prune -c /typo.yaml` used to
// print a warning, drop the flag, and then prune the user's DEFAULT
// database. It must now fail before the command body runs.
...
// TestInitConfig_InvalidConfigFlagExitsNonZero pins the exit code the
// process actually reports. main.go maps Execute's error through
// ExitCodeFor, so this is the observable exit status.
func TestInitConfig_InvalidConfigFlagExitsNonZero(t *testing.T) {
	dir := hermeticConfigEnv(t)
	...
	if got := ExitCodeFor(err); got != 1 {
		t.Errorf("exit code = %d, want 1 (generic failure)", got)
	}
}

// TestInitConfig_DirectoryConfigFlagIsFatal covers the other kit parse
